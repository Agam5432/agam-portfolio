...
export default function Navbar({ active, goTo }) {
  ...
  return (
    <nav className="fixed top-0 left-0 right-0 z-50 h-16 flex items-center justify-between px-6 md:px-12"
      style={{ background: 'rgba(10,10,15,0.88)', backdropFilter: 'blur(16px)', borderBottom: '1px solid #2a2a3a' }}>
      
      {/* Logo */}
      <button onClick={() => handle('home')} className="text-lg font-black tracking-tight gradient-text">
        AT
      </button>
      ...
      <div className="hidden md:flex items-center gap-1">
        {links.map(link => (
          <button
            ...
            style={{ color: active === link.id ? '#00d4ff' : '#888899' }}
          >
            {link.label}
            {active === link.id && (
              <motion.div
      ...
      {/* Mobile Menu Button */}
      <button className="md:hidden text-[#888899]" onClick={() => setOpen(!open)}>
        ...
        >
          {links.map(link => (
            <button
              ...
              style={{
                color: active === link.id ? '#00d4ff' : '#888899',
